...
class UmaApp extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => MaterialApp(
    ...
    theme:ThemeData(
      useMaterial3:true,
      colorScheme:ColorScheme.fromSeed(seedColor:ssumaBlue,brightness:Brightness.light,primary:ssumaBlue,secondary:ssumaGold,surface:Colors.white),
      scaffoldBackgroundColor:ssumaCanvas,
      fontFamily:'Roboto',
      appBarTheme:const AppBarTheme(backgroundColor:ssumaNavy,foregroundColor:Colors.white,elevation:0,centerTitle:false),
      cardTheme:CardThemeData(color:Colors.white,elevation:1,margin:const EdgeInsets.symmetric(vertical:6),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(14))),
      inputDecorationTheme:InputDecorationTheme(filled:true,fillColor:const Color(0xffF8FAFC),border:OutlineInputBorder(borderRadius:BorderRadius.circular(10),borderSide:const BorderSide(color:Color(0xffCBD5E1))),enabledBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(10),borderSide:const BorderSide(color:Color(0xffCBD5E1))),focusedBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(10),borderSide:const BorderSide(color:ssumaBlue,width:2))),
      filledButtonTheme:FilledButtonThemeData(style:FilledButton.styleFrom(backgroundColor:ssumaBlue,foregroundColor:Colors.white,padding:const EdgeInsets.symmetric(horizontal:20,vertical:15),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(10)))),
      navigationBarTheme:const NavigationBarThemeData(backgroundColor:Colors.white,indicatorColor:Color(0xffDCEBFA),labelTextStyle:WidgetStatePropertyAll(TextStyle(fontSize:12,fontWeight:FontWeight.w600))),
      floatingActionButtonTheme:const FloatingActionButtonThemeData(backgroundColor:ssumaBlue,foregroundColor:Colors.white),
      textTheme:ThemeData.light().textTheme.apply(bodyColor:ssumaText,displayColor:ssumaText),
    ),
...
class _SessionGateState extends State<SessionGate>{Widget? page;
 ...
 @override Widget build(BuildContext context)=>page??const Scaffold(backgroundColor:ssumaNavy,body:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[SsumaLogo(size:96),SizedBox(height:18),CircularProgressIndicator(color:ssumaGold)])));
}
                ...
                children: [
                  Container(
                    ...
                    child: const Column(children: [
                      SsumaLogo(),
                      SizedBox(height: 12),
                      Text('SSUMA', style: TextStyle(fontSize: 27, fontWeight: FontWeight.w800, color: Colors.white, letterSpacing: 2)),
                      SizedBox(height: 4),
                      Text('Sistema de seguimiento', style: TextStyle(color: Color(0xffD9E2E8), fontSize: 14)),
                      SizedBox(height: 10),
                      Text('UMA / DOM / SETRAM', style: TextStyle(color: ssumaGold, fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 1)),
                    ]),
                  ),
                  Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        const Text('Registro diario de trabajos', textAlign: TextAlign.center, style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: ssumaText)),
                        const SizedBox(height: 6),
                        const Text('Acceso exclusivo para personal autorizado', textAlign: TextAlign.center, style: TextStyle(color: ssumaMuted, fontSize: 13)),
                        const SizedBox(height: 24),
                        TextField(controller: user, textInputAction: TextInputAction.next, decoration: const InputDecoration(labelText: 'Usuario', prefixIcon: Icon(Icons.person_outline))),
                        const SizedBox(height: 14),
                        TextField(controller: pass, obscureText: true, onSubmitted: (_) => busy ? null : login(), decoration: const InputDecoration(labelText: 'Contraseña', prefixIcon: Icon(Icons.lock_outline))),
                        if (error.isNotEmpty)
                          Container(
                        ...
                        FilledButton(
                          ...
                          child: busy
                              ...
                              : const Text('INGRESAR AL SISTEMA', style: TextStyle(fontWeight: FontWeight.w700)),
                        ),
                        const SizedBox(height: 14),
                        const Text('Gestión institucional 2026', textAlign: TextAlign.center, style: TextStyle(color: ssumaMuted, fontSize: 12)),
                      ],
                    ),
                  ),
                ],
...
class _HomePageState extends State<HomePage>{int index=0; List jobs=[]; Map stats={}; bool busy=true;String period='Hoy';List<String> patios=List.of(patioLocations),buses=List.of(busCodes);
  ...
  @override Widget build(BuildContext context){final pages=[overview(),jobsPage(),const SizedBox(),if(privileged)dashboard(),profile()];return Scaffold(appBar:AppBar(title:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('SSUMA',style:TextStyle(fontSize:18,fontWeight:FontWeight.w800,letterSpacing:1.2)),Text('Registro de trabajos · ${widget.profile['area']}',style:const TextStyle(fontSize:12,fontWeight:FontWeight.w400,color:Color(0xffD9E2E8)))]),actions:[Padding(padding:const EdgeInsets.all(10),child:CircleAvatar(backgroundColor:ssumaGold,foregroundColor:ssumaNavy,child:Text((widget.profile['name']??'U').toString()[0],style:const TextStyle(fontWeight:FontWeight.bold))))]),body:busy?const Center(child:CircularProgressIndicator()):pages[index],floatingActionButton:index==2?null:FloatingActionButton(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>JobForm(token:widget.token,patios:patios,buses:buses))).then((_)=>load()),child:const Icon(Icons.add)),bottomNavigationBar:NavigationBar(selectedIndex:index,onDestinationSelected:(i){if(i==2){Navigator.push(context,MaterialPageRoute(builder:(_)=>JobForm(token:widget.token,patios:patios,buses:buses))).then((_)=>load());}else setState(()=>index=i);},destinations:[const NavigationDestination(icon:Icon(Icons.home_outlined),selectedIcon:Icon(Icons.home),label:'Inicio'),const NavigationDestination(icon:Icon(Icons.history),label:'Trabajos'),const NavigationDestination(icon:Icon(Icons.add_circle_outline),label:'Registrar'),if(privileged)const NavigationDestination(icon:Icon(Icons.bar_chart_outlined),selectedIcon:Icon(Icons.bar_chart),label:'Panel'),const NavigationDestination(icon:Icon(Icons.person_outline),selectedIcon:Icon(Icons.person),label:'Perfil')]));}
  Widget overview()=>ListView(padding:const EdgeInsets.all(18),children:[Text('Buen día, ${widget.profile['name']}',style:const TextStyle(fontSize:26,fontWeight:FontWeight.bold)),Text('${widget.profile['role']} · ${widget.profile['location']??''}',style:const TextStyle(color:ssumaMuted)),if(globalAdmin)const Padding(padding:EdgeInsets.only(top:6),child:Text('Vista general de todo el personal',style:TextStyle(color:ssumaBlue))),const SizedBox(height:22),const Text('Resumen de hoy',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:10),Row(children:[metric('En progreso',todayJobs.where((x)=>x['ESTADO']=='En progreso').length.toString()),const SizedBox(width:10),metric('Finalizados',todayJobs.where((x)=>x['ESTADO']=='Finalizado').length.toString())]),const SizedBox(height:24),Text(privileged?'Trabajos pendientes del personal':'Trabajos por continuar',style:const TextStyle(fontSize:20,fontWeight:FontWeight.bold)),...jobs.where((x)=>x['ESTADO']!='Finalizado').map(jobCard)]);
  Widget metric(String l,String v)=>Expanded(child:Card(color:Colors.white,child:Padding(padding:const EdgeInsets.all(18),child:Column(children:[Text(v,style:const TextStyle(fontSize:28,fontWeight:FontWeight.bold,color:ssumaBlue)),Text(l)]))));
  Widget jobsPage()=>ListView(padding:const EdgeInsets.all(18),children:[Text(globalAdmin?'Trabajos de todo el personal':privileged?'Trabajos del personal del área':'Mis trabajos',style:const TextStyle(fontSize:25,fontWeight:FontWeight.bold)),const SizedBox(height:12),SegmentedButton<String>(segments:const [ButtonSegment(value:'Hoy',label:Text('Hoy'),icon:Icon(Icons.today)),ButtonSegment(value:'Mes',label:Text('Este mes'),icon:Icon(Icons.calendar_month))],selected:{period},onSelectionChanged:(v)=>setState(()=>period=v.first)),const SizedBox(height:14),if(displayedJobs.isEmpty)const Padding(padding:EdgeInsets.all(24),child:Center(child:Text('No hay trabajos registrados en este período',style:TextStyle(color:ssumaMuted)))),...displayedJobs.map(jobCard)]);
  Widget jobCard(dynamic j){
   ...
   return Card(color:Colors.white,margin:const EdgeInsets.symmetric(vertical:7),child:ListTile(
    ...
    isThreeLine:true,
    trailing:Icon(done?Icons.lock:own?Icons.play_circle_fill:Icons.visibility,color:done?const Color(0xff15803D):ssumaBlue),
   ));
  }
  Widget dashboard()=>ListView(padding:const EdgeInsets.all(18),children:[const Text('Dashboard de trabajos',style:TextStyle(fontSize:26,fontWeight:FontWeight.bold)),const SizedBox(height:16),Row(children:[metric('Trabajos','${stats['total']??0}'),const SizedBox(width:10),metric('Finalizados','${stats['completed']??0}')]),const SizedBox(height:10),Row(children:[metric('En progreso','${stats['inProgress']??0}'),const SizedBox(width:10),metric('Horas','${((stats['totalMinutes']??0)/60).toStringAsFixed(1)}')]),if(superAdmin)...[const SizedBox(height:24),const Text('Gestión de Superadministrador',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold,color:ssumaBlue)),const SizedBox(height:10),FilledButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>AdminUsersPage(token:widget.token,patios:patios))),icon:const Icon(Icons.manage_accounts),label:const Text('Gestionar usuarios y personal')),const SizedBox(height:10),OutlinedButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>CatalogAdminPage(token:widget.token))).then((_)=>load()),icon:const Icon(Icons.directions_bus),label:const Text('Gestionar patios y buses'))]]);
  Widget profile()=>ListView(padding:const EdgeInsets.all(24),children:[const CircleAvatar(radius:42,child:Icon(Icons.person,size:42)),const SizedBox(height:14),Text(widget.profile['name']??'',textAlign:TextAlign.center,style:const TextStyle(fontSize:23,fontWeight:FontWeight.bold)),Text(widget.profile['role']??'',textAlign:TextAlign.center,style:const TextStyle(color:ssumaBlue)),const Divider(height:36),ListTile(title:const Text('Usuario'),trailing:Text(widget.profile['username']??'')),ListTile(title:const Text('Área'),trailing:Text(widget.profile['area']??'')),FilledButton.icon(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>ChangePasswordPage(token:widget.token,profile:widget.profile))),icon:const Icon(Icons.password),label:const Text('Cambiar contraseña')),const SizedBox(height:10),OutlinedButton(onPressed:()async{await SessionStore.clear();if(!context.mounted)return;Navigator.pushAndRemoveUntil(context,MaterialPageRoute(builder:(_)=>const LoginPage()),(_)=>false);},child:const Text('Cerrar sesión'))]);
}
...
class _JobFormState extends State<JobForm>{
 ...
 @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:Text(continuing?'Continuar trabajo':'Registrar trabajo')),body:ListView(padding:const EdgeInsets.all(18),children:[
  if(continuing)Card(color:Colors.white,child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('Trabajo original',style:TextStyle(color:ssumaBlue,fontWeight:FontWeight.bold)),const SizedBox(height:6),Text(widget.existing!['DESCRIPCIÓN']?.toString()??''),Text('Avance anterior: $previousProgress% · ${widget.existing!['TIEMPO_TOTAL']??''}',style:const TextStyle(color:ssumaMuted))]))),
